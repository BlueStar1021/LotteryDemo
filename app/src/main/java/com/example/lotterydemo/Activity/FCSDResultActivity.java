...
public class FCSDResultActivity extends NetworkWatcherActivity implements JsonActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_fcsdresult);

        btnRet = (Button)findViewById(R.id.activity_fcsdresult_return);

        no = (TextView)findViewById(R.id.activity_fcsdresult_lottery_no);
        date = (TextView)findViewById(R.id.activity_fcsdresult_lottery_date);
        exdate = (TextView)findViewById(R.id.activity_fcsdresult_lottery_exdate);
        saleAmount = (TextView)findViewById(R.id.activity_fcsdresult_lottery_sale_amount);
        poolAmount = (TextView)findViewById(R.id.activity_fcsdresult_lottery_pool_amount);

        ball_1 = (TextView)findViewById(R.id.activity_fcsdresult_ball_1);
        ball_2 = (TextView)findViewById(R.id.activity_fcsdresult_ball_2);
        ball_3 = (TextView)findViewById(R.id.activity_fcsdresult_ball_3);

        //返回事件监听
        btnRet.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                FCSDResultActivity.super.finish();
            }
        });

        //检查网络
        if(isNetworkAvailable(FCSDResultActivity.this) == false){
            //网络不可用
            Toast.makeText(this, "网络状况不良，请连接数据后重试", Toast.LENGTH_SHORT).show();
        }else{
            //网络可用
            getJson("http://apis.juhe.cn/lottery/query?key=" + getResources().getString(R.string.appkey) + "&lottery_id=fcsd");
        }
    }
